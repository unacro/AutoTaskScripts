...
function Write-Log {
    ...
    $log = ConvertTo-ZhCN "[$($curtime)] $($Level) | $($Content)"
    if ($Level -eq "DEBUG") {
...
function Import-Config {
    ...
    if (Test-Path $config_path) {
        ...
        $xml_writer.WriteAttributeString('version', $script:VERSION)
        ...
        $xml_writer.WriteElementString('app_version', $script:VERSION)
        $xml_writer.WriteElementString('app_updated', $script:UPDATED_AT)
        ...
        $xml_writer.Close()
        if ($first) {
            Import-Config $false # 确保初始化配置文件之后只会再读一次 防止创建失败后进入死循环
        }
    }
}
...
function Get-InputBox {
    [CmdletBinding()]
    param (
        [String]$Prompt = (ConvertTo-ZhCN "Rakuyo \u9996\u53d1\u4e8e\u543e\u7231\u7834\u89e3\u8bba\u575b www.52pojie.cn`n`n
\u8f93\u5165\u60f3\u8981\u89e3\u6790\u76f4\u64ad\u6e90\u7684\u76f4\u64ad\u95f4\u7f51\u5740
\uff08\u5f53\u524d\u7248\u672c\u4ec5\u652f\u6301\u6597\u9c7c\u3001\u864e\u7259\u3001\u0042\u7ad9\uff09"),
        [String]$Title = (ConvertTo-ZhCN "\u76f4\u64ad\u6e90\u83b7\u53d6\u5de5\u5177 v$($script:VERSION) $($script:UPDATED_AT)"),
...
function Get-MsgBox {
    param (
        [String]$Prompt = (ConvertTo-ZhCN "\u8bf7\u8f93\u5165\u6b63\u786e\u683c\u5f0f\u7684\u76f4\u64ad\u95f4\u5730\u5740\uff01"),
        [System.Windows.Forms.MessageBoxButtons]$Buttons = [System.Windows.Forms.MessageBoxButtons]::OK,
        [String]$Title = (ConvertTo-ZhCN "\u83b7\u53d6\u76f4\u64ad\u6e90\u5931\u8d25")
...
function Input-LiveUrl {
    ...
    elseif ($input -eq "") {
        if ($script:METAINFO.never_closed -eq "true") {
            ...
            $first_use = ConvertTo-ZhCN ($first_use += "\u9000\u51fa\u7a0b\u5e8f \u56e0\u6b64\u7a7a\u8f93\u5165\u672c\u7a0b\u5e8f\u4e00\u5f8b\u89c6\u4e3a\u9000\u51fa")
            Write-Host $first_use -ForegroundColor Yellow
            $first_use = "\u8fd9\u4e2a\u63d0\u9192\u53ea\u4f1a\u5728\u7b2c\u4e00\u6b21\u4f7f\u7528\u7684\u65f6\u5019\u663e\u793a\uff0c"
            $first_use = ConvertTo-ZhCN ($first_use += "\u4ee5\u540e\u63a7\u5236\u53f0\u7a97\u53e3\u5c06\u968f\u7740\u7a0b\u5e8f\u7ed3\u675f\u4e00\u8d77\u5173\u95ed\u3002")
...
function Format-LiveUrl {
    ...
    if ($LiveUrl -match ".*=(.*)/?" -and $matches[1].length -gt 0) {
...
function Get-StreamLink {
    if ($null -eq $script:LIVE_ROOM_ID -or $null -eq $script:LIVE_SITE) {
        return $null
    }
    $URI = @{
...
$begin = ConvertTo-ZhCN ($begin += "\u66ff\u4ee3\u4ee5\u4fee\u590d\u8fd9\u4e2abug")
Write-Host $begin
Write-Log -Level DIVIDER
Import-Config
Write-Log -Level DIVIDER
$begin = "\u76f4\u64ad\u6e90\u83b7\u53d6\u5de5\u5177 v$($script:VERSION) \u6700\u540e\u66f4\u65b0\u4e8e$($script:UPDATED_AT)`nRakuyo \u9996\u53d1\u4e8e\u543e\u7231\u7834\u89e3\u8bba\u575b "
...
$title = ConvertTo-ZhCN "\u76f4\u64ad\u6e90\u83b7\u53d6\u6210\u529f"
if ($null -ne $script:LIVE_STREAMER) {
    $streamer = "$($script:LIVE_STREAMER)\u7684"
}
$tips = ConvertTo-ZhCN "\u6210\u529f\u4ece$($streamer)\u76f4\u64ad\u95f4\uff08$($INPUT_URL)\uff09`n"
$tips += ConvertTo-ZhCN "\u83b7\u53d6\u5230\u76f4\u64ad\u6e90 $($stream_link)`n"
$tips += ConvertTo-ZhCN "\u76f4\u64ad\u6e90\u5df2\u7ecf\u590d\u5236\u5230\u526a\u5207\u677f\uff0c\u4f7f\u7528 Ctrl+V \u7c98\u8d34\u3002`n`n"
$tips += ConvertTo-ZhCN "\u5f53\u524d\u9884\u8bbe\u7684\u64ad\u653e\u5668\u4e3a $($script:PLAYER)`n"
$tips += ConvertTo-ZhCN "\u662f\u5426\u4f7f\u7528\u672c\u5730\u64ad\u653e\u5668\u76f4\u63a5\u64ad\u653e\uff1f`n`n"
$tips += ConvertTo-ZhCN "\u70b9\u51fb\u300c\u662f\u300d\u76f4\u63a5\u64ad\u653e`n"
$tips += ConvertTo-ZhCN "\u70b9\u51fb\u300c\u5426\u300d\u751f\u6210.asx\u6587\u4ef6`n"
$tips += ConvertTo-ZhCN "\u70b9\u51fb\u300c\u53d6\u6d88\u300d\u7ed3\u675f\u7a0b\u5e8f`n"
