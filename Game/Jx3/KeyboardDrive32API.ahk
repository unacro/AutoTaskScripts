 ...
 * Version: 1.0.1
 * Description: APIx32 《剑侠情缘网络版叁》按键脚本 32位按键模拟驱动API
 */

class KeyboardDrive32
{
    __New()
    {
        IfNotExist, %A_LineFile%\..\drive\x32\DD.dll
        {
            RegRead, dd_path, HKLM, SOFTWARE\DD XOFT, path
            if !dd_path
            {
                MsgBox, 4112, 错误, 32位按键模拟驱动未找到！
                return
            } else {
                this.hModule := DllCall("LoadLibrary", "Str", dd_path, "Ptr")
                MsgBox, 4144, 警告, 未在脚本目录下找到32位按键模拟驱动，已使用系统驱动！
            }
        } else {
            this.hModule := DllCall("LoadLibrary", "Str", "drive\x32\DD.dll", "Ptr")
            MsgBox, 4160, 提示, 32位按键模拟驱动加载成功！
        }
    }
