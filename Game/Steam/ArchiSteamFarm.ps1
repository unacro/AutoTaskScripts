...
function Copy-Config {
    $importConfirm = Get-MsgBox -Title "准备导入配置文件" -Prompt "若有冲突的文件，是否强制覆盖？`n若选择「否」，则会保留二者中较新的部分。" -Buttons YesNoCancel -Icon Question
...
function Backup-Config {
    if (Test-ASF) {
        $backupConfirm = Get-MsgBox -Title "备份配置" -Prompt "是否备份当前配置文件？" -Buttons YesNo  -Icon Warning
        if ($backupConfirm -eq 'Yes') {
            ...
            Compress-Archive -Path "$($corePath)\config" -DestinationPath "$($workspace)\backup\配置文件备份_$($current)" -Force
        }
    }
}
